...
> 빠짐없이 이어서 진행할 수 있도록 정리했습니다. (작성 기준일: **2026-07-05**)
...
- **v1**: 완료 + 배포됨 (Phase 1~6).
- **v2**: 8단계 계획 중 **P1~P4 완료**, **다음 할 일은 V2-P5**.
- 마지막으로 `npm run build`(= `tsc --noEmit && vite build`)와 `npm test` **모두 통과**
  (테스트 28개 green).
- 커밋되지 않았던 v2 작업(P1~P4)은 이 인수인계와 함께 전부 커밋·푸시됩니다.
...
| **V2-P5** | **응답 값 추출 (`qp_extracted`)** | ⬜ **다음 차례** |
| V2-P6 | 데모 모드 ("샘플 데이터로 체험하기") | ⬜ |
| V2-P7 | 영어 UI 토글 (⚠️ P2 선행 필요 → 이미 충족) | ⬜ |
| V2-P8 | Vercel 마이그레이션 준비 (실제 이전할 때만) | ⬜ |
...
- **CORS 실측 미완:** v1/v2 전체에서 실제 타사 API 5곳 대상 CORS 히트율 점검이
  아직 안 됨(로컬 목 서버로만 검증). 브라우저 전용 전략의 최대 리스크
  (`BLUEPRINT.md` §22). 프록시/클라우드 러너 로드맵 결정 전에 확인 권장.
